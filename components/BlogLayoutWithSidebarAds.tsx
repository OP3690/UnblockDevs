...
import AutoBlogFAQSchema from '@/components/AutoBlogFAQSchema';
...
function ShareButtons() {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <span className="text-[11px] font-semibold uppercase tracking-wide text-zinc-400 mr-1">Share</span>

      {/* WhatsApp */}
      <a href={`https://wa.me/?text=${encodedUrl}`} target="_blank" rel="noopener noreferrer" aria-label="Share on WhatsApp"
        className="inline-flex items-center gap-1 rounded-lg bg-[#25D366] px-2.5 py-2 text-[12px] font-semibold text-white transition-opacity hover:opacity-80">
      ...
      {/* X */}
      <a href={`https://twitter.com/intent/tweet?url=${encodedUrl}&text=${title}`} target="_blank" rel="noopener noreferrer" aria-label="Share on X"
        className="inline-flex items-center gap-1 rounded-lg bg-zinc-900 px-2.5 py-2 text-[12px] font-semibold text-white transition-opacity hover:opacity-80">
        <svg className="h-3 w-3" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
      ...
      <a href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodedUrl}`} target="_blank" rel="noopener noreferrer" aria-label="Share on LinkedIn"
        className="inline-flex items-center gap-1 rounded-lg bg-[#0A66C2] px-2.5 py-2 text-[12px] font-semibold text-white transition-opacity hover:opacity-80">
        <svg className="h-3 w-3" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
      ...
      <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}`} target="_blank" rel="noopener noreferrer" aria-label="Share on Facebook"
        className="inline-flex items-center gap-1 rounded-lg bg-[#1877F2] px-2.5 py-2 text-[12px] font-semibold text-white transition-opacity hover:opacity-80">
        <svg className="h-3 w-3" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
      ...
      {/* Reddit */}
      <a href={`https://www.reddit.com/submit?url=${encodedUrl}&title=${title}`} target="_blank" rel="noopener noreferrer" aria-label="Share on Reddit"
        className="inline-flex items-center gap-1 rounded-lg bg-[#FF4500] px-2.5 py-2 text-[12px] font-semibold text-white transition-opacity hover:opacity-80">
        <svg className="h-3 w-3" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
      ...
      <button onClick={copyLink} aria-label="Copy link"
        className="inline-flex items-center gap-1 rounded-lg border border-zinc-200 bg-white px-2.5 py-2 text-[12px] font-medium text-zinc-600 shadow-sm transition-colors hover:border-zinc-300 hover:bg-zinc-50">
    ...
    <div className="w-full py-6 sm:py-10">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3 px-4 sm:px-6 lg:px-8 xl:px-0 xl:mx-auto xl:max-w-4xl">
        <div className="flex items-center gap-2">
          <Link
            href="/blog"
            className="inline-flex cursor-pointer items-center gap-1.5 rounded-lg bg-zinc-900 px-3 py-2 text-[12.5px] font-semibold text-white shadow-sm transition-colors hover:bg-zinc-700"
          >
          ...
          <Link
            href="/tools/json"
            className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-200 bg-white px-3 py-2 text-[12.5px] font-medium text-zinc-600 shadow-sm transition-colors hover:border-zinc-300 hover:bg-zinc-50 hover:text-zinc-900"
          >
            All Tools
