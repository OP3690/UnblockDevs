...
export default function HomePrivacyFirstSections({
  ...
  return (
    <>
      {/* Tools */}
      <section id="home-tools" className="mx-auto max-w-[1100px] px-6 py-14 sm:py-16">
        <header className="mb-8">
          <span className="font-mono text-[11px] uppercase tracking-[0.12em] text-zinc-500">Tools</span>
          <h2 className="mt-2 text-[26px] font-semibold tracking-[-0.02em] text-zinc-900">Everything you need, daily</h2>
          <p className="mt-2 max-w-[520px] text-[15px] text-zinc-600">
            From JSON formatting to AI-safe data masking. All tools run in your browser with zero server calls for your
            payloads.
          </p>
        </header>

        <div className="mb-6 flex gap-1 overflow-x-auto border-b border-zinc-200 pb-0 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {TABS.map((t) => (
            <button
              key={t.id}
              type="button"
              onClick={() => setCat(t.id)}
              className={`shrink-0 whitespace-nowrap border-b-2 px-4 py-2 text-[13px] font-medium transition-colors ${
                cat === t.id
        ...
        <div className="mb-3 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <Link
            ...
            className="group flex flex-col gap-3 rounded-xl border border-emerald-200 bg-emerald-50/80 p-5 transition-all hover:border-emerald-400 hover:shadow-lg hover:shadow-emerald-900/10"
          >
            <div className="flex items-start justify-between gap-2">
              <div className="flex h-9 w-9 items-center justify-center rounded-lg border border-emerald-200 bg-emerald-50 text-lg">
                🛡️
              </div>
              <div className="flex gap-1">
                <span className="rounded-full bg-blue-50 px-2 py-0.5 font-mono text-[10px] font-medium text-blue-800">
                  AI
                </span>
                <span className="rounded-full bg-amber-100 px-2 py-0.5 font-mono text-[10px] font-medium text-amber-900">
                  Popular
                </span>
              </div>
            </div>
            <div>
              <div className="text-sm font-semibold text-zinc-900">AI Schema Masker</div>
            ...
            <span className="mt-auto flex items-center gap-1 text-[13px] font-medium text-emerald-800 group-hover:gap-1.5">
              Open tool
          ...
          <Link
            ...
            className="group flex flex-col gap-3 rounded-xl border border-zinc-200 bg-white p-5 transition-all hover:border-zinc-300 hover:shadow-md"
          >
            <div className="flex items-start justify-between gap-2">
              <div className="flex h-9 w-9 items-center justify-center rounded-lg border border-blue-200 bg-blue-50 text-lg">
                🔒
              </div>
              <span className="rounded-full bg-blue-50 px-2 py-0.5 font-mono text-[10px] font-medium text-blue-800">AI</span>
            </div>
            <div>
              <div className="text-sm font-semibold text-zinc-900">JSON Prompt Shield</div>
              <p className="mt-1 text-[13px] leading-snug text-zinc-500">
          ...
          <Link
            ...
            className="group flex flex-col gap-3 rounded-xl border border-zinc-200 bg-white p-5 transition-all hover:border-zinc-300 hover:shadow-md sm:col-span-2 lg:col-span-1"
          >
            <div className="flex items-start justify-between gap-2">
              <div className="flex h-9 w-9 items-center justify-center rounded-lg border border-amber-200 bg-amber-50 text-lg">
                🔐
              </div>
              <span className="rounded-full bg-blue-50 px-2 py-0.5 font-mono text-[10px] font-medium text-blue-800">AI</span>
            </div>
            <div>
              <div className="text-sm font-semibold text-zinc-900">Code Prompt Shield</div>
              <p className="mt-1 text-[13px] leading-snug text-zinc-500">
        ...
        <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3 lg:grid-cols-6">
          {visibleMini.map((t) => {
            const dot =
              t.variant === 'ai' ? 'bg-blue-600' : t.variant === 'hot' ? 'bg-amber-500' : 'bg-zinc-300';
            const cellClass =
            ...
            if (t.beautifier) {
              return (
                <button key={t.label} type="button" onClick={onBeautifierClick} className={cellClass}>
            ...
            return (
              <Link key={t.label} href={t.href!} className={cellClass}>
                <span className={`h-1.5 w-1.5 shrink-0 rounded-full ${dot}`} aria-hidden />
                {t.label}
              </Link>
            );
        ...
        <p className="mt-6 text-center">
          <Link href="/tools/json" className="text-sm font-medium text-emerald-800 underline-offset-4 hover:underline">
      ...
      <section className="border-y border-zinc-200 bg-white">
        <div className="mx-auto max-w-[1100px] px-6 py-14 sm:py-16">
          <header className="mb-10">
            <span className="font-mono text-[11px] uppercase tracking-[0.12em] text-zinc-500">How it works</span>
            <h2 className="mt-2 text-[26px] font-semibold tracking-[-0.02em] text-zinc-900">Use AI safely in 4 steps</h2>
            <p className="mt-2 max-w-[520px] text-[15px] text-zinc-600">
          ...
          <div className="grid gap-px overflow-hidden rounded-xl border border-zinc-200 bg-zinc-200 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              <div key={step.n} className="bg-white p-6 sm:p-7">
                <span className="font-mono text-[11px] text-zinc-400">{step.n}</span>
                <div className="mb-3 mt-3 flex h-9 w-9 items-center justify-center rounded-lg bg-zinc-100 text-base">{step.icon}</div>
                <h3 className="text-[15px] font-semibold text-zinc-900">{step.title}</h3>
                <p className="mt-2 text-[13px] leading-relaxed text-zinc-500">{step.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="mx-auto max-w-[1100px] px-6 py-14 sm:py-16">
        <div className="grid gap-10 rounded-xl bg-zinc-950 p-8 sm:grid-cols-2 sm:gap-12 sm:p-12">
          <div>
            <span className="font-mono text-[11px] uppercase tracking-[0.12em] text-zinc-500">Privacy by design</span>
            <h2 className="mt-3 text-[26px] font-semibold leading-snug tracking-[-0.02em] text-white">
              Your data never touches our servers
            </h2>
            <p className="mt-4 text-[15px] leading-relaxed text-zinc-400">
              Every single tool on UnblockDevs runs entirely in your browser. We don&apos;t have servers that process your
              data — because we don&apos;t need them.
            </p>
            <Link
              href={toolPageUrls.schemamasker}
              className="mt-6 inline-flex items-center gap-2 rounded-lg bg-white px-5 py-2.5 text-sm font-medium text-zinc-900 transition-opacity hover:opacity-90"
            >
      ...
      <section className="border-y border-zinc-200 bg-white">
        <div className="mx-auto max-w-[1100px] px-6 py-14 sm:py-16">
          <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <span className="font-mono text-[11px] uppercase tracking-[0.12em] text-zinc-500">From the blog</span>
              <h2 className="mt-2 text-[26px] font-semibold tracking-[-0.02em] text-zinc-900">Guides for developers</h2>
            </div>
            <Link
              href="/blog"
              className="inline-flex w-fit shrink-0 items-center gap-2 rounded-lg border border-zinc-300 bg-white px-4 py-2 text-sm font-medium text-zinc-900 transition-colors hover:bg-zinc-50"
            >
              View all posts
            </Link>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {[
              ...
              <Link
                key={post.href}
                href={post.href}
                className="flex flex-col gap-2 rounded-xl border border-zinc-200 bg-[#FAFAFA] p-5 transition-colors hover:border-zinc-300"
              >
                <span className="font-mono text-[10px] uppercase tracking-wide text-zinc-500">{post.tag}</span>
                <span className="text-[15px] font-semibold leading-snug text-zinc-900">{post.title}</span>
                <p className="text-[13px] leading-relaxed text-zinc-500">{post.desc}</p>
                <span className="mt-auto flex items-center gap-2 pt-2 text-xs text-zinc-400">
                  {post.meta}
