...
export const metadata: Metadata = {
  title: 'Free JSON to TypeScript Generator — Interfaces Online | UnblockDevs',
  description:
    'Convert JSON to TypeScript interfaces, type aliases, or Zod schemas instantly. Handles nested objects, arrays, and null values. Free, 100% browser-based.',
  keywords: [
    'json to typescript',
  ...
  openGraph: {
    title: 'Free JSON to TypeScript Types Generator — Interfaces & Zod Schemas | UnblockDevs',
    description: 'Convert JSON to TypeScript interfaces, type aliases, or Zod schemas instantly. Handles nested objects, arrays, nulls, and optional fields. 100% browser-based, nothing uploaded.',
    type: 'website',
    url: canonicalUrl,
    images: [{ url: 'https://unblockdevs.com/api/og?title=JSON%20to%20TypeScript%20Types%20Generator&emoji=%7B%7D&desc=Convert%20JSON%20to%20TypeScript%20interfaces%2C%20type%20aliases%2C%20or%20Zod%20schemas%20instantly', width: 1200, height: 630, alt: 'JSON to TypeScript Types Generator — UnblockDevs' }],
  },
  twitter: {
    card: 'summary_large_image',
    title: 'JSON to TypeScript Generator — Free Interfaces & Zod Schemas Online',
    description: 'Paste JSON → get TypeScript interfaces instantly. Handles nested objects, arrays, null values, and Zod schemas. 100% browser-based.',
  },
...
const faqSchema = {
  ...
  mainEntity: [
    ...
    { '@type': 'Question', name: 'Why does TypeScript not catch bad API response shapes at runtime?', acceptedAnswer: { '@type': 'Answer', text: 'TypeScript types are erased at compile time — they do not validate data at runtime. JSON.parse() returns any, so even a perfectly typed interface gives you no protection against an API returning an unexpected shape. The solution is runtime validation: generate a Zod schema from your JSON using this tool and call schema.parse(JSON.parse(responseText)). Zod throws if the data does not match, giving you both runtime safety and TypeScript type inference.' } },
    ...
    { '@type': 'Question', name: 'How do I keep my TypeScript types in sync when the API schema changes?', acceptedAnswer: { '@type': 'Answer', text: 'Re-paste the updated API response into this generator whenever the schema changes — regenerating takes seconds. For production teams, consider generating types from an OpenAPI spec using openapi-typescript (npx openapi-typescript schema.yaml --output types.ts) so types are always in sync with the documented contract. Use Zod schemas generated here for runtime validation alongside the TypeScript types.' } },
      ...
      <ToolSEOContent>
        ...
        <SEOSection id="faq" eyebrow="FAQ" heading="Frequently Asked Questions">
          <FAQ items={[
            ...
            { q: 'Why does TypeScript not catch bad API response shapes at runtime?', a: <>TypeScript types are erased at compile time — they do not validate data at runtime. <C>JSON.parse()</C> returns <C>any</C>, so even a perfectly typed interface gives no protection against an unexpected API shape. Generate a Zod schema here and call <C>schema.parse(JSON.parse(responseText))</C> for runtime safety with TypeScript inference.</> },
            ...
            { q: 'How do I keep my TypeScript types in sync when the API schema changes?', a: 'Re-paste the updated API response into this generator whenever the schema changes — regenerating takes seconds. For teams, use openapi-typescript (npx openapi-typescript schema.yaml --output types.ts) to generate types directly from an OpenAPI spec so they always match the documented contract.' },
