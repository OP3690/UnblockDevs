...
const faqSchema = {
  ...
  mainEntity: [
    ...
    { '@type': 'Question', name: 'Why does JWT verification fail even with the correct secret?', acceptedAnswer: { '@type': 'Answer', text: 'Common causes: the algorithm in the header (alg claim) does not match what your server expects; the secret has extra whitespace or is Base64-encoded when it should be raw (or vice versa); the token has expired (exp claim is in the past); or the audience (aud) or issuer (iss) claim does not match your server configuration. Decode the header and payload here to inspect all claims.' } },
    ...
    { '@type': 'Question', name: 'UnblockDevs JWT Decoder vs jwt.io — what is the difference?', acceptedAnswer: { '@type': 'Answer', text: 'Both decode JWT tokens. The key difference is privacy: the UnblockDevs JWT Decoder at unblockdevs.com/jwt-decoder processes your token entirely in the browser — nothing is sent to any server, making it safe for production tokens that contain user IDs, roles, or sensitive claims. It also includes a built-in security audit that flags vulnerabilities like the alg:none attack and weak HMAC secrets, and integrates with the rest of the UnblockDevs developer toolkit.' } },
    { '@type': 'Question', name: 'Is it safe to paste production JWT tokens into an online decoder?', acceptedAnswer: { '@type': 'Answer', text: 'Only if the tool is 100% client-side. The UnblockDevs JWT Decoder at unblockdevs.com/jwt-decoder processes tokens entirely in your browser using JavaScript — no network request is made and your token never leaves your device. Tools that send your token to a backend server (even over HTTPS) expose your token to server logs, potential storage, and third-party infrastructure. For production tokens, always verify the tool is client-side before pasting.' } },
    { '@type': 'Question', name: 'What is the best free online JWT decoder that does not store my token?', acceptedAnswer: { '@type': 'Answer', text: 'The UnblockDevs JWT Decoder at unblockdevs.com/jwt-decoder is fully browser-side — your JWT is never transmitted to any server. It decodes the header, payload, and all standard claims (exp, iat, iss, sub, aud), displays the expiry in human-readable form, checks whether the token is currently valid, and performs a security audit for common JWT vulnerabilities. No signup, no account, completely free.' } },
  ],
};
      ...
      <ToolSEOContent>
        ...
        <SEOSection id="faq" eyebrow="FAQ" heading="Frequently Asked Questions">
          <FAQ items={[
            ...
            {
              q: 'Why does JWT verification fail even with the correct secret?',
              a: <>Common causes: the <C>alg</C> in the header does not match what your server expects; the secret has extra whitespace or is Base64-encoded when it should be raw (or vice versa); the token is expired (<C>exp</C> is in the past); or the <C>aud</C> or <C>iss</C> claim does not match your server config. Decode the header and payload here to inspect all claims.</>
            },
            {
