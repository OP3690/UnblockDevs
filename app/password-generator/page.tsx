...
export default function PasswordGeneratorPage() {
  return (
    <>
          ...
          <FAQ items={[
            {
              ...
              a: 'At minimum 12 characters for general accounts; 16+ for sensitive accounts like email, banking, and admin panels. Each extra character exponentially increases brute-force difficulty. The entropy meter shows exact crack-time estimates at 1 billion guesses per second.',
            },
            {
              q: 'What makes a password strong?',
              a: 'High entropy: long length, a large character set (uppercase, lowercase, numbers, symbols), and no predictable patterns. Avoid dictionary words, names, dates, and keyboard walks. Aim for 80+ bits of entropy for sensitive accounts.',
            },
            {
              q: 'Passphrase vs password — which is better?',
              a: 'Both can be equally strong. A 4-word passphrase has ~44 bits per word from a large wordlist, making it very strong and far easier to remember than a random 16-character string. Use passphrases for master passwords you must memorise; use random passwords for everything stored in a password manager.',
            },
            {
              q: 'Is it safe to use an online password generator?',
              a: 'Yes, when it runs entirely in your browser. This tool uses window.crypto.getRandomValues() — the cryptographically secure RNG built into every modern browser. No password is sent to any server. Disconnect from the internet before generating to verify.',
            },
            {
              q: 'Can I generate API keys or JWT secrets here?',
              a: 'Yes. The secret key generator section produces hex, base64url, and other encodings suitable for JWT signing, API keys, OAuth secrets, and AES keys. All keys are generated client-side — copy them into environment variables and never commit to version control.',
            },
            {
              q: 'What is the Have I Been Pwned breach check?',
              a: 'It uses the k-anonymity model from the HaveIBeenPwned API. Only the first 5 characters of your password\'s SHA-1 hash are sent to the API. Your actual password is never transmitted.',
            },
          ]} />
        </SEOSection>
      ...
      <ToolPageFooterBand toolName="password_generator" />
    </>
  );
}
