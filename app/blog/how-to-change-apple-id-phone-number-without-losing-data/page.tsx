import type { Metadata } from 'next';
import HowToChangeAppleIdPhoneNumberClient from './client';

export const metadata: Metadata = {
  title: 'Change Apple ID Phone Number Without Losing Data 2026 | UnblockDevs',
  description: 'Change Apple ID phone number without losing data. iPhone, iPad, Mac. Step-by-step. 2026.',
  keywords: [
    'change apple id phone number',
    'change apple id phone number without losing data',
    'how to change apple id phone number',
    'update apple id phone number',
    'change apple id phone number iphone',
    'apple id phone number change 2026',
    'change apple id phone number safely',
    'apple id phone number update guide',
    'change apple id contact info',
    'update apple id phone number step by step',
    'apple id phone number change tutorial',
    'change apple id phone number mac',
    'apple id phone number change ipad',
    'apple id account settings',
    'apple id phone number change guide'
  ],
  openGraph: {
    title: 'How to Change Apple ID Phone Number Without Losing Data: Complete Guide 2026',
    description: 'Change Apple ID phone number without losing data. Step-by-step for all devices.',
    type: 'article',
    publishedTime: '2026-01-25T00:00:00Z',
    authors: ['UnblockDevs'],
    url: 'https://unblockdevs.com/blog/how-to-change-apple-id-phone-number-without-losing-data',
    images: [{ url: 'https://unblockdevs.com/api/og?title=How%20to%20Change%20Apple%20ID%20Phone%20Number%20Without%20Losing%20Data%3A%20Complete%20Guide%202026&emoji=%F0%9F%9B%A0%EF%B8%8F&desc=Change%20Apple%20ID%20phone%20number%20without%20losing%20data', width: 1200, height: 630, alt: 'How to Change Apple ID Phone Number Without Losing Data: Complete Guide 2026 — UnblockDevs Blog' }],

  },  twitter: {
    card: 'summary_large_image',
    title: 'How to Change Apple ID Phone Number Without Losing Data: Complete Guide 2026',
    description: 'Change Apple ID phone number without losing data. Step-by-step for all devices.',
  },
  alternates: { canonical: 'https://unblockdevs.com/blog/how-to-change-apple-id-phone-number-without-losing-data' },

};

export default function HowToChangeAppleIdPhoneNumberGuide() {
  return <HowToChangeAppleIdPhoneNumberClient />;
}
