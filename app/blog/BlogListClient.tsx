...
export function BlogListClient({ initialPosts, totalPages, currentPage }: BlogListClientProps) {
  ...
  return (
    <>
      {/* Results count — helpful UX signal */}
      <div className="mb-5 flex items-center justify-between">
        <p className="text-[12px] text-zinc-400 font-medium">
          {initialPosts.length === 0
            ? 'No posts found — try browsing all categories.'
            : `Showing ${initialPosts.length} post${initialPosts.length !== 1 ? 's' : ''}${totalPages > 1 ? ` · Page ${currentPage} of ${totalPages}` : ''}`}
        </p>
      </div>
      ...
      {initialPosts.length === 0 ? (
        <div className="py-16 flex flex-col items-center text-center">
          ...
          <h3 className="text-[15px] font-bold text-zinc-900">No posts found</h3>
