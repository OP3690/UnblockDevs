...
export const metadata: Metadata = {
  ...
  keywords: [
    ...
    // extended cluster
    'uuid v1 generator', 'uuid v3 generator', 'uuid v5 generator', 'uuid v6 generator',
    'uuid v8 generator', 'generate uuid free', 'uuid online tool',
    'random uuid', 'unique id generator', 'uuid format',
    'uuid batch generator', 'uuid lowercase uppercase', 'uuid without hyphens',
    'uuid copy paste', 'uuid javascript', 'crypto.randomUUID',
    'uuid npm package', 'uuid python', 'uuid java', 'guid c#', 'uuid golang',
    'uuid postgres', 'gen_random_uuid postgres', 'uuid mysql', 'uuid mongodb',
    'uuid vs ulid', 'ulid generator', 'nanoid generator', 'cuid generator',
    'what is uuid', 'uuid structure', 'uuid collision probability',
    'uuid uniqueness', 'is uuid truly unique', 'uuid version 1 vs 4',
    'uuid time based', 'uuid random', 'rfc 4122', 'uuid nil',
...
const faqSchema = {
  ...
  mainEntity: [
    ...
    { '@type': 'Question', name: 'How do I validate a UUID format?', acceptedAnswer: { '@type': 'Answer', text: 'A valid UUID matches the pattern: /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i. Paste into our UUID Validator to check format and identify the version.' } },
    { '@type': 'Question', name: 'What is UUID v7 and why is it better for databases?', acceptedAnswer: { '@type': 'Answer', text: 'UUID v7 encodes a Unix millisecond timestamp in the first 48 bits, making IDs sort chronologically. This means database inserts are sequential rather than random, dramatically reducing index fragmentation and improving write performance compared to v4.' } },
    { '@type': 'Question', name: 'What is the difference between UUID and ULID?', acceptedAnswer: { '@type': 'Answer', text: 'ULID (Universally Unique Lexicographically Sortable Identifier) is 128-bit like UUID but encoded in Crockford Base32 (26 chars vs 36 for UUID). ULIDs are time-sortable by default and URL-safe. UUID v7 achieves similar time-sortability while maintaining the standard UUID format.' } },
    { '@type': 'Question', name: 'How do I generate a UUID without hyphens?', acceptedAnswer: { '@type': 'Answer', text: 'Generate a UUID and remove the hyphens: uuid.replaceAll("-", "") in JavaScript. Or: uuid.uuid4().hex in Python. Some databases (MySQL, MongoDB) store UUIDs without hyphens in BINARY(16) columns for efficiency.' } },
  ],
};
